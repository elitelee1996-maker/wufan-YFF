...
def validate_skill(skill_path):
    ...
    if dir_name and not re.match(r'^[a-z0-9][a-z0-9-]*[a-z0-9]$|^[a-z0-9]$', dir_name):
        return False, f"目录名 '{dir_name}' 应为 slug 格式（小写字母、数字和连字符，不以连字符开头/结尾）"
    if dir_name and '--' in dir_name:
        return False, f"目录名 '{dir_name}' 不能包含连续连字符"
